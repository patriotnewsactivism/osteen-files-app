...
type Registry = Record<string, { title: string }>;
export default function EvidenceCard(
  { item, registry }: { item: EvidenceItem; registry: Registry }
) {
  return (
    <article id={item.id} className="scroll-mt-24 rounded-2xl border p-4 md:p-6 shadow-sm bg-white/70 backdrop-blur">
      ...
      {item.docPath && (
        <details className="mt-2">
          ...
          <div className="mt-2 aspect-[4/3] w-full">
            <iframe title={item.title} src={item.docPath} className="w-full h-full rounded-lg border" />
          </div>
        </details>
      )}
