export type EvidenceItem = {
  ...
  title: string;
  /**
   * Optional date associated with the piece of evidence. Use ISO-ish strings
   * (e.g. "Aug 11, 2023") so they're easy to read.
   */
  date?: string;
  /**
   * A simple type discriminator to help render different evidence kinds
   * consistently in the UI. Extend this union if you add new kinds.
   */
  kind: "PDF" | "Transcript" | "Court Filing" | "Summary";
  /**
   * Relative path to the document served from `/public/docs` when present.
   */
  docPath?: string;
  /**
   * List of external links associated with this evidence. These might be
   * off-site videos (e.g. body‑worn camera footage) or other hosted files.
   */
  externalLinks?: { label: string; url: string }[];
  /**
   * Key points extracted from the evidence. Keep each entry concise.
   */
  highlights: string[];
  /**
   * Inline citation tags rendered as small chips. These can reference
   * whatever citation system you adopt. They are kept as arbitrary strings
   * so the consumer of this data can decide how to interpret them.
   */
  citationTags: string[];
  /**
   * Identifiers of other evidence items that relate to this one. Used to
   * produce cross‑navigation chips in the UI so readers can hop between
   * connected documents.
   */
  relatedIds?: string[];
};
